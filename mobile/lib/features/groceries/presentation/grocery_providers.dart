...
class RecommendedGroceriesState {
  final bool isLoading;
  final String? error;
  final List<Grocery> items;
  final int totalCount;
  ...
  const RecommendedGroceriesState({
    this.isLoading = false,
    this.error,
    this.items = const [],
    this.totalCount = 0,
  ...
  RecommendedGroceriesState copyWith({
    bool? isLoading,
    String? error,
    List<Grocery>? items,
    int? totalCount,
    ...
    return RecommendedGroceriesState(
      isLoading: isLoading ?? this.isLoading,
      error: error,
      items: items ?? this.items,
      totalCount: totalCount ?? this.totalCount,
...
class RecommendedGroceriesController extends Notifier<RecommendedGroceriesState> {
  late final GroceryRepo _repo;

  @override
  RecommendedGroceriesState build() {
    _repo = ref.read(groceryRepoProvider);
    return const RecommendedGroceriesState();
  }
  ...
  Future<void> _loadPage({
    ...
    try {
      final result = await _repo.recommendedGroceries(
        page: page,
        pageSize: state.pageSize,
      );
...
final recommendedGroceriesControllerProvider =
    NotifierProvider.autoDispose<RecommendedGroceriesController, RecommendedGroceriesState>(
  RecommendedGroceriesController.new,
);
